...
module G = Api.Interface.Gen(PicoRV32.I)(PicoRV32.O)
module G' = HardCamlLlvmsim.Sim.Gen(B)(PicoRV32.I)(PicoRV32.O)
...
let init_testbench () = 
  ...
  let circ,sim,i,o = 
    if llvm then
      G'.make "opicorv32" PicoRV32.f 
    else
      let circ,sim,i,o,_ = G.make "opicorv32" PicoRV32.f in
      circ,sim,i,o
  in
...
let run_testbench (sim, i, o, memory, waves) = 
  let open PicoRV32.I in
  let open PicoRV32.O in
